    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_bleu >= best_val_bleu:
            ...
            torch.save(
                ...
                os.path.join(save_dir, f"{exp}.pth"),
            )

        torch.cuda.empty_cache()

    checkpoint = torch.load(os.path.join(save_dir, f"{exp}.pth"), map_location=device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()

    print(f"\nLoaded best checkpoint from {exp}.pth\n")
...
if __name__ == "__main__":
    ...
    fig.suptitle(f"Training Curves — {exp}", fontsize=14)
