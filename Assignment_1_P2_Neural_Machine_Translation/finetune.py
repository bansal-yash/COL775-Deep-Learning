...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for src, trg, src_mask in tqdm(
            ...
            if isinstance(model, seq2seq_bert):
                outputs = model(src, src_mask, trg, teacher_forcing_ratio=0.6)
            elif isinstance(model, seq2seq_attention):
                outputs = model(src, trg, teacher_forcing_ratio=0.6, src_mask=src_mask)
            else:
        ...
        if val_bleu >= best_val_bleu:
            ...
            torch.save(
                ...
                os.path.join(save_dir, f"{exp}_finetuned.pth"),
            )

        torch.cuda.empty_cache()

    checkpoint = torch.load(os.path.join(save_dir, f"{exp}_finetuned.pth"), map_location=device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()

    print(f"\nLoaded best checkpoint from {exp}_finetuned.pth\n")
...
if __name__ == "__main__":
    ...
    if args.pretrained_model is not None:
        pretrained_ckpt = torch.load(args.pretrained_model, map_location=device)
        ...
        filtered_sd = {k: v for k, v in pretrained_sd.items() if k not in skip_keys}

        missing, unexpected = model.load_state_dict(filtered_sd, strict=False)
        print(f"Loaded pretrained weights from {args.pretrained_model}")
        print(
            f"  Skipped (vocab-dependent) : {sorted(skip_keys & pretrained_sd.keys())}"
        )
        print(f"  Missing in checkpoint     : {sorted(missing)}")
        print(f"  Unexpected in checkpoint  : {sorted(unexpected)}\n")
    ...
    with open(os.path.join(save_dir, f"metrics_{exp}_finetuned.json"), "w") as f:
        json.dump(round_metrics(metrics), f, indent=4)

    epochs_range = range(1, EPOCHS + 1)
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(f"Training Curves — {exp} Finetuned", fontsize=14)
    ...
    for ax, (key, ylabel, title) in zip(axes.flat, plot_configs):
        plt.sca(ax)
        if key == "losses":
            plot_metrics(epochs_range, metrics[key][0], metrics[key][1], ylabel, title)
        else:
            plt.plot(epochs_range, metrics[key], label="Validation")
            plt.xlabel("Epochs")
            plt.ylabel(ylabel)
            plt.title(title)
            plt.legend()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"metrics_{exp}_finetuned.png"), dpi=150)
    plt.close()
